...
class RankingController(BaseController):
    ...
    def get_expert_rankings(self):
        ...
        # 1. PENTING: Kita tetap ambil SEMUA data untuk mendapatkan nilai MAX (Normalisasi)
        # dan Rata-rata Bobot Global yang valid.
        results = db.query(ExpertProfile, ExpertWeight, User.email)\
        ...
        if not results:
            return {
                "global_average_weights": {
                    "education": 0, "patient": 0, "publication": 0, "flight_hours": 0
                },
                "rankings": []
            }
        ...
        for profile, weight, email in results:
            ...
            processed_data.append({
                "email": email,
                "user_id": profile.user_id, # Tambahkan user_id untuk filtering
                "raw": {
                    "education": edu_score,
        ...
        for item in processed_data:
            ...
            ranked_experts.append({
                "user_id": item["user_id"], # Dipakai untuk filter di bawah
        ...
        # 5. FILTER: Hanya kembalikan data milik current_user
        # Kita filter list hasil akhir berdasarkan user_id dari token
        my_ranking_data = [
            expert for expert in ranked_experts 
        ]
        
        # Opsional: Kita hapus field 'user_id' dari output agar bersih (karena sudah difilter)
        for item in my_ranking_data:
            item.pop("user_id", None)

        return {
            "global_average_weights": avg_weights,
            "rankings": my_ranking_data
        }
        
        
    def get_my_expert_rankings(self, current_user):
        db: Session = next(self.get_session())
